...
def simulate_spurious_pac(n_points, fs, spike_amp=1.5, spike_fwhm=0.01,
    ...
    return spikes + noise, spikes, noise
...
# generate the signal
signal, spikes, noise = simulate_spurious_pac(
...
axs[0].set(xlabel='Time (sec)', title='spikes + pink noise')
axs[1].plot(time, spikes[:n_points_plot], color='C1')
axs[1].set(xlabel='Time (sec)', title='spikes')
axs[2].plot(time, noise[:n_points_plot], color='C2')
axs[2].set(xlabel='Time (sec)', title='noise')
plt.show()
